...
class MtxCollection(BaseModel):
    ...
    storage_directory: Union[Path, S3Path]
    ...
    @classmethod
    def ensure_typing_of_path(cls, raw: str) -> Union[S3Path, Path]:
    ...
    def list_studies(self) -> List[Union[Path, S3Path]]:
    ...
    def list_samples(self, study_name: str) -> List[Union[Path, S3Path]]:
    ...
    @staticmethod
    def clean_listdir(
        path: Union[Path, S3Path], ignore_patterns: List[str] = [".DS_Store", ".log"]
    ) -> Generator[Union[Path, S3Path], None, None]:
        """
        A generator that yields directory contents, ignoring specific files or patterns.

        Args:
            path (Union[Path, S3Path]): The directory to list contents from.
            ignore_patterns (List[str]): List of patterns to ignore (e.g., ".DS_Store").

        Yields:
            Union[Path, S3Path]: Valid directory components.
        """
        if not path.is_dir():
            path = path.to_dir()
    ...
    def read_mtx(
        self, root_fp: Union[Path, S3Path], files: Optional[List[str]] = None
    ...
    def read_metadata_file(self, fp: Union[Path, S3Path], reindex_columns: List[str]) -> pd.DataFrame:
        try:
    ...
    @staticmethod
    def read_csv(filepath: Union[S3Path, Path], **kwargs) -> pd.DataFrame:
        # Long form of reading versus supplying path because that's how S3Path reads are supported
